...
<!--     <xsl:template match="button[@id = 'circular-key']" mode="ixsl:onclick">
    ...
    <xsl:template match="button[@id = 'page-base-uri']" mode="ixsl:onclick">
        ...
        <xsl:for-each select="id('base-uri', ixsl:page())">
            <xsl:result-document href="?." method="ixsl:append-content">
                ...
                document-uri(ixsl:page()): <xsl:value-of select="document-uri(ixsl:page())"/><br/>
                base-uri($button): <xsl:value-of select="base-uri($button)"/><br/>
                ixsl:get($button, 'baseURI'): <xsl:value-of select="ixsl:get($button, 'baseURI')"/><br/>
            </xsl:result-document>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="button[@id = 'doc-base-uri']" mode="ixsl:onclick">
        <xsl:for-each select="id('base-uri', ixsl:page())">
            <xsl:result-document href="?." method="ixsl:append-content">
                ...
                Doc document-uri(): <xsl:value-of select="document-uri(document('https://namedgraph.github.io/saxon-js2-test/test.xml'))"/><br/>
            </xsl:result-document>
        </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template name="docLoaded">
        ...
        <xsl:for-each select="id('base-uri', ixsl:page())">
            <xsl:result-document href="?." method="ixsl:append-content">
                ...
                Async doc document-uri(): <xsl:value-of select="document-uri($body)"/><br/>
    ...
    <xsl:template match="button[@id = 'updating-function']" mode="ixsl:onclick">
        <xsl:variable name="callback-name" select="QName('https://w3id.org/atomgraph/client#', 'ac:update-document')" as="xs:string"/>
